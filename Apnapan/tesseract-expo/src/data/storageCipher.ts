...
async function key(create: boolean): Promise<AESEncryptionKey> {
  ...
  if (!creating) {
    creating = (async () => {
      ...
      await SecureStore.setItemAsync(KEY_NAME, await generated.encoded('base64'), {
...
export async function openStorage(address: string, raw: string): Promise<string> {
  ...
  try {
    const bytes = await aesDecryptAsync(AESSealedData.fromCombined(raw.slice(MARKER.length)), await key(false), {
      additionalData: new TextEncoder().encode(address),
    });
