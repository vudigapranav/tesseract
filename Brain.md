...
## 2026-09-08 Apnapan branding + remaining Expo features

Product is now **Apnapan** (अपनापन). Team attribution unchanged: "Built and
developed by the Tesseract Team." Flutter untouched.
...
- `apnapan-icon.png` — 1024px, mark at 62% (≈19% margin), for the app icon
- `apnapan-icon-adaptive.png` — 1024px, mark at 44%, for Android's mask
- `apnapan-mark.png` — 512px, for the native splash plugin

First attempt widened the crop instead and the Hindi wordmark bled into the
icon; composing onto a canvas rather than widening a crop is what fixes it.

`app.json`: `name` → Apnapan, icons wired. **`slug`, `bundleIdentifier` and
`scheme` deliberately unchanged** — renaming them would break Firebase, the API
identity and any existing install, and none of that is part of a rename.

### Opening screen

`src/screens/OpeningScreen.tsx` + `src/state/useOpeningScreen.ts`. Logo, the
tagline as **real localised text**, and the attribution. ~1s hold, subtle fade
skipped under reduce-motion, cream ground matched to the artwork (#FEF8EE).

The interruption-vs-reopen problem is the substance here. iOS sends the same
events for a Face ID prompt and a real departure, so a naive implementation
flashes the brand every time biometrics appear — mid-activity, that interrupts
someone. Two discriminators: **`inactive` never counts** (biometrics, permission
sheets, the notification shade all sit there; only a real departure reaches
`background`), and a **90s minimum away**. Six tests cover the boundary.

Shown as an **overlay over a still-mounted tree**, so route, form text, auth and
a running game keep state and timing. Loading runs in parallel with the hold;
if not ready when the hold ends it says so rather than routing prematurely.

### Expo Go vs installed build — verified, not assumed

`expo-doctor` rejected the top-level `splash` key: it is legacy in SDK 57 and
the current API is the `expo-splash-screen` plugin. Fixed. Expo docs also
confirm **since SDK 52 Expo Go shows its own app icon instead of the splash**,
which is precisely why the in-app opening screen exists. The Apnapan
home-screen icon and native launch appearance need a separately installed
build; Expo Go remains the container. No dev client was introduced.

### Features completed this pass

- Draft badges/notices removed from the **first-run** language picker (kept in
  Settings, where the information is actionable). Data model unchanged.
- Noto fonts bundled and loaded.
- **Know Me** — places, steps, words, sortable pictures in the caregiver's own
  script, with conflict-aware upload. Feeds real content into games; below the
  threshold a game uses the generic set and **says so** rather than presenting a
  half-personal mixture as personal. Sessions now report the real content
  revision instead of a hardcoded '1'.
- **Reminders** — create/edit/enable/delete/acknowledge/postpone, local
  notifications in the patient's language with the caregiver's exact wording,
  permission at point of use, schedule reconciled by cancel-then-rebuild so a
  restart cannot duplicate or orphan.
- **Recommendations** — deterministic and explainable. Refuses to suggest below
  4 finished sessions, states the counts behind every suggestion, and is harder
  to trigger upward than downward. Accept / modify / reject, with 409 keeping
  local edits.
- **Doctor screens** — server-decided assignment, observed measures shown as
  absent when absent (never a fabricated zero), attributed notes.
- **Biometric caregiver gate** — device auth where enrolled; where not, it says
  so instead of pretending, and still requires a deliberate press.

### Verified

TypeScript strict clean · **50 tests passing** · `expo-doctor` 21/21 · iOS
bundle builds · opening screen and sign-in **captured on the iOS Simulator**
(iPhone 17 Pro, iOS 26.5, Expo Go 57.0.9). Simulator evidence only — **not an
iPhone**. Nothing has been verified on real hardware.
